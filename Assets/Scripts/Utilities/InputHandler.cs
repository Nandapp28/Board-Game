...
public class JoystickController : MonoBehaviour
{
    ...
    public string portName = "COM4"; // Change this to the correct port (e.g., COM3 on Windows or /dev/ttyUSB0 on Linux/Mac)
    ...
    void Update()
    {
        if (serialPort.IsOpen)
        {
            try
            {
                ...
                if (joystickX > 1900 + threshold) // Gerakan ke kanan
                {
                    Debug.Log("Gerakan: Kanan");
                }
                else if (joystickX < 1900 - threshold) // Gerakan ke kiri
                {
                    Debug.Log("Gerakan: Kiri");
                }

                if (joystickY > 1900 + threshold) // Gerakan ke atas
                {
                    Debug.Log("Gerakan: Bawah");
                }
                else if (joystickY < 1900 - threshold) // Gerakan ke bawah
                {
